...
import {
  ...
  type ReportStoreFormState,
} from "@/server/actions/report-actions";

const initialState: ReportStoreFormState = {};

type ReportStoreFormProps = {
  storeId: string;
  storeSlug: string;
  csrfToken: string;
};
...
const REPORT_REASONS = [
  ...
  { value: "MISLEADING_OR_UNSAFE", label: "Misleading or unsafe" },
  { value: "SPAM", label: "Spam" },
  { value: "IMPERSONATION", label: "Impersonation" },
] as const;

export function ReportStoreForm({
  storeId,
  storeSlug,
  csrfToken,
}: ReportStoreFormProps) {
  ...
  return (
    <form action={formAction} className="rounded-2xl border border-[var(--border)] bg-[var(--surface-alt)] p-4">
      <div>
        <h3 className="text-sm font-semibold text-[var(--text)]">Report this store</h3>
        <p className="mt-1 text-xs leading-5 text-[var(--text-muted)]">
          Flag impersonation, spam, unsafe claims, or other issues for moderation review.
        </p>
      </div>
