...
export async function submitResourceReportAction(
  ...
  try {
    ...
    if (!userRateLimit.success || !ipRateLimit.success) {
      ...
      return {
        error: `Too many reports submitted. Please wait ${retryAfter} seconds and try again.`,
      };
    }
    ...
    logger.error("submitResourceReportAction failed.", error);
...
export async function submitStoreReportAction(
  ...
  try {
    ...
    logger.error("submitStoreReportAction failed.", error);
    return { error: "Something went wrong. Please try again." };
  }
}
