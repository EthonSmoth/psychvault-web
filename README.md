...
- **Logbook parser at `/logbook`** — pay-per-parse ($2/credit) tool for AHPRA 5+1 internship logbooks; supports LBPP-76 (XLSX + PDF), CHPS-76, INPP-76, PACF-76; stores zero logbook content in the database; all parsed data lives in browser React state only
...
Run this in the Supabase SQL Editor to create the logbook payment-tracking tables (required before `/logbook` features work):
...
These tables store only Stripe payment records and per-parse receipts. No logbook content is ever written to the database.
...
src/
  ...
  app/
    ...
    api/
      ...
      logbook/checkout/       Stripe Checkout session for parse-credit purchase
      logbook/webhook/        Stripe webhook for logbook credit fulfilment
      logbook/credits/        GET — authenticated user credit balance
      logbook/parse/          POST multipart — XLSX/PDF logbook parser (consumes 1 credit)
      upload/                 Image upload + WebP optimization
      contact/                Contact form
      admin/                  Admin API routes
    logbook/                  Marketing page — logbook parser product page
    logbook/dashboard/        Auth-required dashboard — upload, parse, visualise, download
  ...
  lib/
    ...
    logbook-constants.ts      AHPRA 5+1 internship hour requirements (single source of truth)
...
- `LOGBOOK_STRIPE_WEBHOOK_SECRET` — signing secret for the **second** Stripe webhook endpoint pointing at `/api/logbook/webhook` (separate from `STRIPE_WEBHOOK_SECRET`); PDF parsing uses `pdf-parse` (free, no external API key needed)
